...
@NoArgsConstructor
@Schema(
    title = "Run async ES|QL query",
    description = "Executes an async ES|QL query and returns hits in different formats if query is completed before end of `waitForCompletionTimeout`. Otherwise returns a query ID. Defaults to `fetchType=FETCH`; STORE writes results to Kestra internal storage. Only the current result set is processed—no pagination."
)
@Plugin(
    metrics = {
    ...
    examples = {
        @Example(
            title = "Load data in bulk to Elasticsearch and query it using async ES|QL.",
            full = true,
            code = """
                ...
                tasks:
                  ...
                  - id: query
                    type: io.kestra.plugin.elasticsearch.AsyncEsqlRun
                    fetchType: STORE
                    query: |
                      FROM books
...
public class AsyncEsql extends AbstractTask implements RunnableTask<AsyncEsql.Output> {
    ...
    @Override
    public AsyncEsql.Output run(RunContext runContext) throws Exception {
        ...
        try (ElasticsearchClient client = this.connection.highLevelClient(runContext)) {
            ...
            CompletableFuture<Iterable<Map<String, Object>>> queryResponse = esqlAsyncClient.query(ForkObjectsEsqlAdapter.of(TYPE_REFERENCE.getType()), queryRequest);

            Output.OutputBuilder outputBuilder = AsyncEsql.Output.builder();

            switch (runContext.render(this.fetchType).as(FetchType.class).orElseThrow()) {
                case FETCH:
                    Pair<List<Map<String, Object>>, Integer> fetch = this.fetch(queryResponse.get());
                    outputBuilder
                        .rows(fetch.getLeft())
                        .size(fetch.getRight());
                    break;
                ...
                case FETCH_ONE:
                    var o = this.fetchOne(queryResponse.get());
                ...
                case STORE:
                    Pair<URI, Long> store = this.store(runContext, queryResponse.get());
                    outputBuilder
                        .uri(store.getLeft())
            ...
            int size = Iterables.size(queryResponse.get());
